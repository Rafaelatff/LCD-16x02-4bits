...
void send_LCD_nibble(char data);
void start_LCD_4bit_pins(void);
...
void send_LCD_nibble(char data){
    if(data & 0x01) P6OUT |= BIT0; //Compare D4
        else P6OUT &= ~BIT0;
    if(data & 0x02) P3OUT |= BIT3; //Compare D5
        else P3OUT &= ~BIT3;
    if(data & 0x04) P6OUT |= BIT1; //Compare D6
        else P6OUT &= ~BIT1;
    if(data & 0x08) P6OUT |= BIT2; //Compare D7 (MSB)
        else P6OUT &= ~BIT2;
...
void send_LCD_byte(char RS, char data){
    ...
    send_LCD_nibble(data>>4); // Send high part of data
    send_LCD_nibble(data & 0x0f);// Clean high part of data and send low part
    delay_ms(2);
}
